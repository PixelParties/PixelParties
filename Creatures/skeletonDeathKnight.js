...
export class SkeletonDeathKnightCreature {
    ...
    async executeDarkSlashAttack(deathKnightActor, target, position) {
        ...
        this.applyDarkSlashDamage(target);
        ...
        this.battleManager.addCombatLog(
            `💥 The dark slash strikes true, dealing ${this.SLASH_DAMAGE} damage and silencing the target!`, 
            'info'
        );
    }
    ...
    createDarkSlashEffect(fromElement, toElement) {
        ...
        try {
            ...
            if (fromRect.width === 0 || fromRect.height === 0 || toRect.width === 0 || toRect.height === 0) {
                console.warn('Cannot create slash effect: elements have invalid dimensions');
                return null;
            }
            ...
            if (!isFinite(fromX) || !isFinite(fromY) || !isFinite(toX) || !isFinite(toY)) {
                console.warn('Invalid slash coordinates detected:', { fromX, fromY, toX, toY });
                return null;
            }

            // Calculate slash properties
            const deltaX = toX - fromX;
            const deltaY = toY - fromY;
            const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
            
            // Prevent slashes that are too short or too long (likely errors)
            if (distance < 10 || distance > 2000) {
                console.warn(`Invalid slash distance: ${distance}px`);
                return null;
            }
            
            const angle = Math.atan2(deltaY, deltaX) * (180 / Math.PI);

            // Create the dark slash effect element
            ...
            console.log(`Created dark slash: ${distance.toFixed(1)}px at ${angle.toFixed(1)}°, animation time: ${adjustedAnimationTime}ms`);
            return slashEffect;
    ...
    async createGuestDarkSlash(deathKnightData, targetData, animationTime, myAbsoluteSide) {
        ...
        if (targetElement) {
            ...
            // Wait for slash to reach target, then show impact
            await this.battleManager.delay(adjustedAnimationTime);
    ...
    async executeDeathSlashStormAttack(deathKnightCreature, heroOwner, position, deathKnightSide, allTargets) {
        ...
        allTargets.forEach((target, index) => {
            ...
            slashPromises.push(this.executeDeathSlash(deathKnightElement, target, index));
        });
