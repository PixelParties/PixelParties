...
export class ThepHeroEffect {
    ...
    static async checkThepRevival(dyingHero, battleManager) {
        ...
        // Perform revival
        dyingHero.currentHp = revivalHp;
        dyingHero.alive = true;
        
        // Mark hero as revived by Thep (cannot be revived again)
        if (!dyingHero.customStats) {
            dyingHero.customStats = {};
        }
        dyingHero.customStats.revivedByThep = true;
        
        // Update health bar immediately
        battleManager.updateHeroHealthBar(
            dyingHero.side, 
            dyingHero.position, 
            dyingHero.currentHp, 
            dyingHero.maxHp
        ...
        const heroElement = battleManager.getHeroElement(dyingHero.side, dyingHero.position);
        if (heroElement) {
        ...
        await this.createThepRevivalAnimation(dyingHero, battleManager);
        
        // Log the revival
        battleManager.addCombatLog(
            `💚 Thep's divine protection revives ${dyingHero.name} with ${revivalHp} HP! (${graveyardSize} cards in graveyard)`,
            dyingHero.side === 'player' ? 'success' : 'error'
        );
    ...
    /**
     * Create green revival animation for Thep's protection
     * @param {Object} hero - Hero being revived
     * @param {Object} battleManager - Battle manager instance
     */
    static async createThepRevivalAnimation(hero, battleManager) {
    ...
    static handleGuestThepRevival(data, battleManager) {
        ...
        const heroElement = battleManager.getHeroElement(dyingHeroLocalSide, dyingHeroPosition);
        if (heroElement) {
            const card = heroElement.querySelector('.battle-hero-card');
            if (card) {
                card.classList.remove('defeated');
        ...
        // Create revival animation
        this.createThepRevivalAnimation(dyingHero, battleManager);
